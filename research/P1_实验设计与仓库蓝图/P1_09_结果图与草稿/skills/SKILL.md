---
name: P1_09_结果图与草稿_node_skill
description: Node-local strategy skill for `research/P1_实验设计与仓库蓝图/P1_09_结果图与草稿`. Use
  only when this node's tier requires `skills/SKILL.md`.
---

# P1_09_结果图与草稿 Node Skill

This node-local skill applies to `research/P1_实验设计与仓库蓝图/P1_09_结果图与草稿`.
...
- When `artifacts/figure_plan.yaml 缺失`, keep_default_delegate.
- When `artifacts/figure_plan.yaml 已存在但 artifacts/claim_figure_map.yaml 缺失`, delegate to canonical worker `result_to_claim`.
- When `上游 claim_map 缺失`, stop with `waiting_for_claim_map`.
