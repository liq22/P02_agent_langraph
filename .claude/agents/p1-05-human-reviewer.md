---
name: p1-05-human-reviewer
description: User-authorized Claude Code teammate delegate for P1_05 human-review-slot review. Use only for the P1_05 review/人类_001.md gate after the user explicitly approves sending the P1_05 review packet to Claude Code/Anthropic.
...
Review `research/P1_实验设计与仓库蓝图/P1_05_初步验证结果整理` for the human-review slot after the user explicitly approves sending the P1_05 review packet to Claude Code/Anthropic.
...
- `research/P1_实验设计与仓库蓝图/P1_05_初步验证结果整理/review/human_reviewer_prompt.md`
...
2. Write a v2 Claude Code handoff to `docs/submission_ready_goal/runtime_logs/claude_code/p1_05_human_review_handoff.yaml`.
3. Run:

```bash
python tools/submission_ready_goal/validate_claude_handoff.py --handoff docs/submission_ready_goal/runtime_logs/claude_code/p1_05_human_review_handoff.yaml
```
...
If you recommend pass, still state the remaining non-node blockers separately: selected backend, RM101 Stage B reject evidence, PHMGA adapter preflight, and downstream Stage C/D rows are not solved by P1_05 human review.
