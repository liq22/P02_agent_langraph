---
name: p4-03-human-reviewer
description: User-authorized Claude Code teammate delegate for P4_03 human-review-slot review. Use only for the P4_03 review/人类_001.md gate after user authorization for Claude Code/Anthropic teammate review.
...
Review `research/P4_论文回复_response/P4_03_逐点回复草稿_md` for the human-review slot after user authorization for Claude Code/Anthropic teammate review.
...
- `research/P4_论文回复_response/P4_03_逐点回复草稿_md/review/human_reviewer_prompt.md`
...
2. Write a v2 Claude Code handoff to `docs/submission_ready_goal/runtime_logs/claude_code/p4_03_human_review_handoff.yaml`.
3. Run:

```bash
python tools/submission_ready_goal/validate_claude_handoff.py --handoff docs/submission_ready_goal/runtime_logs/claude_code/p4_03_human_review_handoff.yaml
```
...
If you recommend pass, still state remaining non-node blockers separately: official journal comments are absent, official editor comment is absent, P4_05/P4_06/P4_07 are not solved by P4_03, mapped blocking actions remain open, review-score thresholds are below final submission threshold, selected backend and PHMGA/Vibench formal evidence are unresolved, and final submission validation still fails.
