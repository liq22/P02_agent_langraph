...
The extension protocols remain at provider-free readiness. Horizon-v3 projects the dynamic-v3 target-adverse endpoint and matched bearing-cluster inference over 144 units; its validate-only schedule emits 144/144 dry commands with zero executions. Reliability-v2 registers 80 matched pairs/160 episode bundles; its dry schedule emits 160/160 inert commands, invokes none, and performs zero provider calls and writes. Its analyzer makes target-adverse replay Average Precision primary: each repeat recomputes AP over all 24 assigned windows from registered private DataPort targets and canonical rollout submit prefixes, while grounded pass@1 and pass-all-10 remain explanatory. It rejects the legacy hyphenated attempt layout and does not use derived `evaluation.jsonl` rows as target or prediction authority. Its accepted-result consumer rechecks all 160 bundles, 80 pairs, ten repeat-level task-primary deltas, pass projections, and selected rollout/cost outcomes before a grouped table/figure/manuscript update; analyzer/scheduler checks pass 7/7, consumer checks pass 9/9, and it has emitted zero rows. Cross-dataset-v3 registers the accepted Ottawa ordered-state target and CSV DataPort; its provider-free preflight emits 18/18 commands for 72 episode bundles, 36 matched pairs, and 216 assigned windows. Its accepted-only analyzer independently rebuilds private assignments and requires 36 exact pairs before a 2,000-resample bearing-clustered analysis. Its accepted-result consumer rechecks the 72-bundle/36-pair gate, 108 assigned windows per arm, displayed task deltas, and physical-bearing bootstrap metadata before a grouped table/figure/manuscript update; the prior chain passes 21/21, the consumer passes 9/9, and it has emitted zero rows. No P2-E8 command has run or result exists, and external provider execution remains unauthorized. The dynamic-v3 protocol retains horizons 3, 6, and 12 plus the full and four ablation profiles. Its accepted-only analyzer reconstructs the eight evaluator-private 12-window masters through the registered Paderborn DataPort, projects exact h3/h6 prefixes across every seed and arm, and reads predictions from canonical rollout successful-submit prefixes; runner-derived `evaluation.jsonl` is not target or prediction authority. Its accepted result now carries the opaque seed/sequence projections needed to reproduce secondary paired-cluster arithmetic. The consumer rechecks the 240-unit gate, eight task-primary rows, and 26 separately labeled P2-E3--P2-E7 mechanism rows before a grouped table/task-primary-figure/manuscript update. It recomputes mechanism deltas, bootstrap intervals, exact sign tests, defined numerators, and per-metric Holm families; P2-E4 supplies the reused P2-E7 no-branching comparison without duplicating an episode denominator. Its 11/11 focused checks and a real 240-unit analyzer-to-consumer fixture pass, and it has emitted zero rows. The dedicated formal runner is statically ready, the provider-free schedule emits 240/240 commands and invokes none, and validate-only reads no provider environment or probe evidence and performs no provider call or filesystem write. Formal coverage is 0/240; formal-runner checks pass 17/17 and dynamic-focused checks pass 50/50. Complete accepted cohorts remain the entry condition for horizon, ablation, cross-dataset, reliability, and Graph-effect estimates.
...
| Core Graph state diagnostics | `paper/experiments/results/graph_core_state_primary.json`; `paper/assets/tables/graph_core_state_primary.md` | Graph-only state integrity on diagnosis and anomaly episodes |
...
| Replay state diagnostics | `paper/experiments/results/graph_monitor_state_primary.json`; `paper/assets/tables/graph_monitor_state_primary.md` | eight-state occupancy, visitation, and transition integrity; zero Monitor/Revise values are valid because v6 registers no `public_condition_event` |
| Replay mechanism case | `paper/experiments/results/graph_monitor_mechanism_case.json`; `paper/assets/figures/graph_monitor_mechanism_case.svg` | one deterministically selected descriptive long-horizon semantic/state pair |
...
*Pending: the accepted core comparison and descriptive replay semantic/state case are inserted only after all matched core and replay result contracts pass.*
