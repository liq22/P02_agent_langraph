...
This paper isolates the effect of explicit decision structure. The control is the Benchmark Generic policy; `ReactiveSequentialAgent` is its zero-behavior-override wrapper and is therefore Reactive-equivalent rather than a separate learned or prompted agent. `GraphDecisionAgent` is derived directly from the same Generic base and adds the eight-state decision control. Neither production arm imports P1 runtime code. The arms share data, operators, numerical experts, the global catalog, action and output budgets, and evaluator capabilities. The Graph arm's state suffix and filtered schema exposure are the joint treatment and can change realized input-token use, which is reported as an outcome. Graph states are recorded in the shared trajectory and must affect actual next-action selection.

Prior work establishes explicit state tracking, graph/state-machine workflow structure, and state-conditioned tool selection or filtering. This study contributes a registered matched PHM intervention: deterministic state derived only from public Benchmark trajectory fields, state-specific visibility over the unchanged tool catalog, a zero-override Generic control, condition-event and topology ablations, and task scoring by an independent evaluator. The graph constrains high-level state and legal actions while the shared LLM policy still chooses the within-state tool trajectory.
...
ReAct interleaves reasoning and environment actions so an agent can update plans and handle exceptions [@yao2023react], while Reflexion uses linguistic feedback and episodic memory to influence subsequent trials [@shinn2023reflexion]. StateAct augments a base agent with goal-reinforcing self-prompting and a chain of tracked state information across steps [@rozanov2025stateact]. AgentBench shows that long-horizon reasoning and decision-making remain common failure sources across interactive environments [@liu2023agentbench]. The registered P2 treatment computes its decision state deterministically from public Benchmark trajectory fields, records that state with each canonical action, and filters visible schemas from the unchanged global tool catalog.
...
StateFlow is the direct state-machine precedent. It models LLM task solving as a state machine, separates process grounding through states and transitions from actions within a state, and permits rule- or LLM-selected transitions over context history [@wu2024stateflow]. The P2 experimental question is narrower: whether deterministic public-rollout state and state-specific tool visibility change independently evaluated PHM task outcomes when the Generic base, computational world, budget, and episode order are matched.

HyperAgent is the direct schema-planning comparison: it constructs a tool--schema hypergraph and task DAG, then expands a state-conditioned support graph for the current subtask [@zhai2026hyperagent]. P2 applies a fixed eight-state PHM decision policy to the shared Benchmark catalog; state comes from recorded public actions and observations, and the intervention constructs neither a task DAG nor a schema-dependency hypergraph.

STAR externalizes multi-agent specialist routing as a state-conditioned transition policy over the current agent, task type, and typed execution status, combining expert-specified nominal routes with recovery transitions learned from execution traces [@yang2026star]. Each P2 arm runs a single Generic-derived policy per episode, with no specialist pool or inter-agent router, so its paired estimand isolates deterministic within-agent graph guidance over public PHM rollout fields.
...
![The shared eight-state topology with distinct legal-transition matrices: the 51-edge base-v6 relation, in which Monitor/Revise are unreachable because the active primary registers no public condition event, and the 33-edge dynamic-full relation whose provider-bound formal cohort has not run.](../assets/figures/graph_policy_states.svg)
...
The control and treatment use the same Benchmark Generic base prompt, model runtime, task instances, global tool catalog, numerical experts, action and output budgets, and stochastic seeds. The control is reported as Benchmark Generic (Reactive-equivalent); `ReactiveSequentialAgent` changes no Generic behavior. `GraphDecisionAgent` adds the registered current-state prompt suffix and state-specific visibility over that catalog. This joint intervention changes the realized prompt and visible-schema surface by design; input tokens and cost are measured outcomes, and the study does not identify suffix length and schema filtering as separate effects.

Core diagnosis and anomaly comparisons use the Paderborn bearing dataset [@lessmeier2016conditionMonitoring; @paderbornBearingDataCenter] with all four bearing-grouped rotations, the sample at metadata-order index $\lfloor 2(n-1)/3 \rfloor$ from each of the 32 held-out bearings, and three paired seeds. These formal records are distinct from the exact midpoint records used during pre-formal endpoint and feature-contract development. Every bounded read contains all 8,192 samples from channel index 2, the bearing-housing vibration column mapped by the public upstream reader; the shared full-rate window contract prevents the material high-frequency aliasing identified by the benchmark sampling audit. Replay stress uses the eight held-out bearings in rotation 0, three ordered windows per bearing, and the same seeds. Fold-level expert/reference fitting and validation selection occur before the matched episodes and are identical across policies; the reported cost comparison therefore covers inference rollouts only. Core episodes have a 33-call budget; replay episodes allow 72 calls, three reads, 50 operator calls, and three model calls. These limits are shared by the Benchmark Generic and Graph arms.

The provider-bound primary is registered in the Benchmark-owned 45-job P1/P2 joint schedule under profile `p1-p2-joint-primary-v1`; the existing Paper-0 B3 prefix is neither a control nor poolable with it. Generic, PHMskills, and Graph cycle through all three execution positions across 12 core and three monitoring units. Generic is executed once per unit and serves as this paper's Reactive-equivalent control; no second Reactive provider job is run. Every runner validates and persists the same exact 13-field schedule identity. A Graph-first job validates the frozen local Benchmark/Data-Factory topology without claiming that its later same-unit Generic comparison already exists. Benchmark now locally implements a serialized exact-prefix inspector/executor whose scheduler, acceptance, and executor contract suite passes 43/43; its provider-free check derives progress from canonical artifacts, and one gated execution can start at most one job. This implementation grants no provider permission. The formal source revisions remain dirty or unpublished, P1/P2 joint provider/public-context egress has not been separately authorized in addition to the base authorization, no fresh exact-route/model zero-price two-turn admission report is bound, and 0/45 jobs have run. The schedule therefore remains `execution_ready=false`, no joint acceptance artifact exists, and no treatment result is available.

The extension protocols keep the same task-primary hierarchy and acceptance discipline. Dynamic-v3, horizon-v3, Ottawa UORED-VAFCLS v5 P2-E8 [@sehri2023uoredVafclsV5; @sehri2023uoredVafclsDataBrief], and reliability P2-E9 all retain assigned windows and failed or partial episodes, use physical-bearing inference where registered, and admit manuscript estimates only from complete accepted cohorts. Their current execution and validation status is reported once in Section 6 and the claim--evidence matrix.
...
| Dynamic-v3 | Benchmark Generic (Reactive-equivalent) plus the full and four Graph profiles | 10/10 exact-six Mock cells, 0 provider calls; dedicated formal runner 19/19 and dynamic-related checks 57/57 | runner ready; 240/240 dry-run commands emitted, 0 invoked; 0/240 formal units; formal gate not accepted | Event routing and profile mechanics only |
...
The P2-E1 finalizer consumes explicit active-v0.2 timestamped roots and validates each `cohort_index.json` against canonical exact-six leaves. The checked-in readiness artifact records `external_roots_required_no_audit_performed`, carries no retired-root counts, and emits no effect estimate. Every Graph unit must bind its exact joint-schedule identity and the clean Benchmark/Data Factory/P2 source topology; its same-unit Generic control may execute before or after it according to the registered counterbalance, while result acceptance still requires both completed arms. Once all four arm gates and both pairing gates accept, the finalizer emits four absolute arm summaries and two paired bearing-bootstrap results in `p2_e1_generic_base_formal_v2_result.json`. It also projects all 24 exact replay pairs from public canonical actions and evaluator rollout metrics, retaining non-submitted and natural-failure denominators and reporting Graph state and transition counts without hidden-target selection, reasoning traces, or a post-hoc case. The accepted-only renderer independently verifies that identity, every displayed estimate, interval, valid-replicate count, and the full-cohort mechanism arithmetic. Focused finalizer and renderer checks pass, including a complete 192-core/24-replay-per-arm fixture.
...
| Replay full-cohort mechanism projection | `paper/experiments/results/p2_e1_replay_mechanism_v1.json` (generated after acceptance; currently absent) | reconcile public canonical actions and evaluator rollout endpoints across all 24 exact pairs; report Graph state and transition integrity without selecting a case |
...
Formal trajectories require 192 unique episodes in each Generic-base core arm and 24 in each monitoring arm, aligned by seed, fold, bearing, sample, and task. The accepted replay projection will cover all 24 pairs and summarize submission, budget exhaustion, valid and repeated calls, steps, and grounded recovery; state occupancy, episode visitation, and transition validity describe Graph treatment integrity. No post-hoc replay case is selected. The v6 primary defines no `public_condition_event`. Observation-conditioned Monitor/Revise behavior and dynamic edge ablations belong to the separate `paderborn_graph_dynamic_ablation_v3` profile, whose retained Mock gate is mechanics-only and whose formal coverage is 0/240.

## 10. Reproducibility

The authoritative task and split configuration is the Benchmark production protocol at `../p01-phm-agent-benchmark/paper/experiments/datasets/dataset_protocol.yaml`; Goal-pack copies are not experiment inputs. `ReactiveSequentialAgent` and `GraphDecisionAgent` derive directly from `GenericLLMToolAgent`; the former is the zero-override Benchmark Generic control, while the latter adds registered graph decision control. `src/phm_graph_agent/state.py` defines the eight-state relation, and `scripts/run_graph_experiment.py` is the single-arm implementation entry point. The Benchmark P0-only launcher is `../p01-phm-agent-benchmark/paper/experiments/run_formal_paper0_v6.sh`; its B3 profile and roots are ineligible for the downstream study and cannot be pooled with it. The Benchmark-owned provider-free projection `../p01-phm-agent-benchmark/paper/experiments/schedule_downstream_formal_v2.py` defines a fresh 45-job joint counterbalance over 15 units and the Generic, PHMskills, and Graph arms: 36 core jobs and nine monitoring jobs. Generic executes once per unit and supplies the shared control for both P1 and P2. `../p01-phm-agent-benchmark/paper/experiments/execute_joint_primary_schedule_v1.py` now provides a provider-free exact-prefix check and a separately gated, locked one-job execution mode, with 43/43 focused scheduler/acceptance/executor checks passing locally. It does not authorize inference. Dirty or unpublished formal source revisions, missing separate P1/P2 joint egress authorization, a missing fresh admission report, and 0/45 executed jobs keep `execution_ready=false` and `provider_calls=0`. Each episode attempt contains the canonical exact-six files, and provider errors remain immutable leaves. The provider-free P02 suites cover implementation, source topology, private-assignment authority, accepted-run provenance, metric denominators, cluster inference, valid-bootstrap reporting, task/mechanism separation, P2-E7 claim boundaries, and protocol-bound atomic rendering. `scripts/render_current_mechanics_evidence.py` regenerates the current mechanics table and figure. The final ten-lens review begins after formal results, final tables and figures, and a result-grounded conclusion are available.

## 11. Discussion and Limitations

The graph is hand-authored, deterministic, and small. The study covers one vibration dataset family, two core tasks, a replay stress protocol, and one primary LLM runtime. The treatment couples state text with state-specific tool-schema filtering, so the estimated intervention is their joint graph-guidance effect; its realized prompt length and visible-schema token surface differ from the control and the design does not isolate those components. The v6 primary registers no `public_condition_event`; Monitor/Revise values there are integrity diagnostics. Dynamic-v3 has retained provider-free mechanics but no provider-bound cohort. Effects can vary with episode length and Generic-base model quality. The shared experts use a short fixed window and generic time/band features without shaft-speed or bearing-geometry fault-frequency verification, which limits physics-grounded interpretation. Free-endpoint queueing, load, and backend drift make latency descriptive.
...
The Paper 2 implementation, registered protocols, and accepted provider-free mechanics artifacts are maintained in the public [P02 repository](https://github.com/liq22/P02_agent_langraph) under Apache-2.0. The active artifacts remain on Draft PR #2 and the current working snapshot rather than a stable default-branch archive. Executable task, evaluator, and joint-scheduling contracts are maintained in the private upstream [Benchmark repository](https://github.com/liq22/phm-agent-benchmark), and data-access adapters are maintained in the public upstream [Data Factory repository](https://github.com/PHMbench/phm-data-factory). Independent reconstruction currently requires authorized Benchmark access. No archival release or DOI exists.

Raw Paderborn and Ottawa UORED-VAFCLS v5 signals remain at their official licensed sources and are not redistributed by P02: Paderborn under CC BY-NC 4.0 [@paderbornBearingDataCenter] and Ottawa under CC BY 4.0 [@sehri2023uoredVafclsV5]. Local source paths are supplied only at execution. Protocol-bound preparation and analysis code reconstructs evaluator-private targets and source-to-opaque-handle crosswalks from those local copies. Public task observations and rollouts retain opaque handles and exclude local paths, evaluator targets, and the private crosswalks.

Accepted provider-bound Graph results are not yet available: the joint primary has executed 0/45 jobs, dynamic-v3 has observed 0/240 formal units, Ottawa P2-E8 has executed 0/72 registered episode bundles, and reliability P2-E9 has executed 0/160 registered episode bundles. These repositories therefore contain no accepted Generic-versus-Graph treatment estimate.
