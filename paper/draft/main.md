...
**Observation completeness precedes numerical comparison.** Before executing any selected ancestor, check that the recorded support contains usable window count/rate, nonempty finite feature vectors, and the prediction's model identity, sample identity, class and nonempty finite probability map. Graph-support validity alone does not provide these observations. An incomplete or unusable original record leaves replay unresolved without a replay call; it does not narrow the comparison to whichever fields remain. The same observation requirements apply to recomputed results. Missing values are not evidence of either agreement or numerical disagreement.

For each corresponding finite feature or probability scalar $v$ and its recomputed value $v'$, agreement requires

$$
|v'-v|\le\max\!\left\{\epsilon_a,\epsilon_r\max(|v|,|v'|)\right\},
\qquad \epsilon_r=10^{-9},\quad\epsilon_a=10^{-12}.
$$

Vector lengths and probability keys must match. Class, model/sample identities and recorded window metadata are compared exactly; the numeric tolerance cannot excuse changed input scope. A finite observed difference beyond these rules is a mismatch. A missing or unusable recomputed observation is unresolved, and execution failures are retained separately. These comparisons define reproduction of the declared public observations, not bitwise equality across hardware or every hidden intermediate array. Offline replay calls and cost are reported separately and never become free online evidence.

## 4. Testable consequences

Evidence feedback should be behaviorally relevant where call-history progress and numerical readiness differ: incomplete feature roles, wrong-window ancestry, premature submission or a useful remaining expert prediction. Removing a required usage edge while retaining all node values should invalidate support and can alter the next admitted menu. A failed call should not create evidence. A lossless table conversion should preserve structural predicates even when its model-facing tokenization differs. Removing recorded feature values or prediction probabilities must prevent a replay-agreement claim even if every dependency edge remains intact; this observation-completeness test is distinct from edge corruption. On a shared ready post-prediction prefix, $10^+$ and $11$ should match the continuation menu; a supported but schema-undiscovered prefix tests the precise limit of that match.
...
For graph support, count submitted predictions with complete, role-bound raw-to-prediction ancestry. For replay, report support-valid paths, paths passing the recorded-observation check, eligible paths with available execution assets, attempted paths, and their matched, mismatched, failed or unresolved outcomes. An incomplete original record is unresolved and unattempted; an incomplete recomputed result is unresolved after an attempt. Neither disappears from the assigned cohort, and a conditional agreement rate must identify its denominator and unresolved coverage. A useful summary is the fraction of final submitted diagnoses having both valid recorded support and successful fresh replay; report its coverage over all assigned episodes as well. This joint rate does not replace the separate native grounding, path validity and replay rates, and neither implies a correct fault label.
