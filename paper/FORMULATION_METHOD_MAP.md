...
| Replay observability and agreement | Required recorded/recomputed observations; finite-value tolerance | Figure 2 evaluator-side output; 2.3/3.5 | After step 7, `replay_prediction` preflight and comparison | D3: missing values unresolved before calls; finite changes mismatched; scope exact |
...
Fresh replay uses the same raw identity/slice/channels/rate and fitted assets in a declared numerical environment. It never calls the LLM, refits or substitutes cached features as computed outputs. Feature/probability tolerance is rtol=1e−9, atol=1e−12; class, identities and recorded scope are compared exactly. These are comparison rules, not cross-hardware determinism guarantees. Cached-start replay validates only its downstream segment. Grounding, replay, physical fault causality and hidden-reasoning faithfulness are different claims.

Replay additionally requires observable comparison values. Before any replay call, the selected original support must contain usable window count/rate, nonempty finite feature-vector values and a prediction identity/class/nonempty finite probability map. Missing or unusable original observations raise an unresolved-replay ValueError without calling tools; missing or unusable recomputed observations remain unresolved after execution. Finite observed differences produce `matched=False`. A supported path with missing values must not pass by silently comparing fewer fields. This measurement correction changes neither `prediction_supported` nor the online frontier, policies, treatments or diagnostic scores. The native grounded cohort aggregation remains separately pending.

## Figure roles and regeneration

Figure 1 remains unchanged: motivation/problem variables and common native validation. Figure 2 retains the decision–evidence loop and the independent 10/10⁺/11 comparison strip, not new execution stages. The public W/E predicates, η, s, Γ and N are matched to Section 3.4 and Algorithm 1. Replay observability belongs to its existing evaluator-side output, not a new feedback path. The sole generator remains `scripts/figures/plot_phmgraph_formulation_method.py` in Benchmark; editable SVG, PDF and PNG derive from it. Both SVG assets and drawing source are unchanged in this revision. No third figure or duplicated drawing implementation is added.

## Source binding and actual validation

Merged Benchmark source: `eceada18ebe18ef1e3b8210830b326cc4343c6b7`, PR #35. Tested head: `fd17dad2e896e89600ee14069eb22d404265ce4e`; tested merge ref: `007eb13d84e16da926a8bd75ed669971f64f8575`. The underlying dual method is PR #27, the continuation comparator PR #30, and the exact control maps PR #32. This revision changes only replay observation checks and their tests; online treatment behavior is unchanged.

Final installed workflow `35427170354` passed 51 focused tests. Workflow `35427170356` passed 262 research tests and 52 selected portable core tests; one pre-existing real-HDF5 acceptance test was explicitly deselected. The suites overlap. Actual Factory submodule `58050716383e32ca79fdad0d9a45ad96a19eb838` was installed and `pip check` passed. The seven added leaf test methods reproduce the incomplete-record false positive and cover missing/nonfinite original and recomputed observations, finite mismatches and scope-sensitive tolerance. The earlier six applicability-fixture errors remain fixed. No production validator was weakened.
